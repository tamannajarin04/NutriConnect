...
Revision ID: 9a316131de1f
Revises: 
Create Date: 2026-03-15 05:17:41.182223
...
def upgrade():
    ...
    # food_items: add as nullable first, backfill, then enforce constraints
    with op.batch_alter_table('food_items', schema=None) as batch_op:
        batch_op.add_column(sa.Column('availability_status', sa.String(length=20), nullable=True, server_default='available'))
        batch_op.add_column(sa.Column('order_count', sa.Integer(), nullable=True, server_default='0'))
        batch_op.add_column(sa.Column('view_count', sa.Integer(), nullable=True, server_default='0'))
        batch_op.add_column(sa.Column('updated_at', sa.DateTime(), nullable=True))

    op.execute("UPDATE food_items SET availability_status = 'available' WHERE availability_status IS NULL")
    op.execute("UPDATE food_items SET order_count = 0 WHERE order_count IS NULL")
    op.execute("UPDATE food_items SET view_count = 0 WHERE view_count IS NULL")
    ...
    # users: add as nullable first, backfill, then enforce constraints
    with op.batch_alter_table('users', schema=None) as batch_op:
        batch_op.add_column(sa.Column('is_verified', sa.Boolean(), nullable=True, server_default=sa.false()))
